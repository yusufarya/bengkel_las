            ...
            <tbody>
                <?php foreach ($data['kategori'] as $key => $val) { ?>
                    <tr style="font-size: 13px;">
                        ...
                        <td style="text-align: right;"><?= number_format($val['biaya'], 2) ?> &nbsp;</td>
                        <td style="text-align: center;">
                            <a href="#" onclick="editData('<?= $val['id'] ?>','<?= $val['nama'] ?>', '<?= $val['keterangan'] ?>', '<?= $val['biaya'] ?>', <?= $level_ ?>)" class="text-warning bg-white"><i class="bi bi-pencil"></i></a> &nbsp;
            ...
            <form action="<?= base_url('addKategori') ?>" method="POST">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="biaya" name="biaya" placeholder="0">
                    </div>
                </div>
...
<div class="modal fade" id="editData" tabindex="-1" aria-labelledby="editDataLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="editDataLabel">Ubah Data Kategori</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="<?= base_url('Master/updateKategori') ?>" method="POST">
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col mb-3">
                            <label for="biaya_edit" class="form-label">Biaya</label>
                            <input type="text" class="form-control" id="biaya_edit" name="biaya_edit" required placeholder="0">
                        </div>
                    </div>
                </div>
                <div class="modal-footer">

...
class Login extends CI_Controller
{
    ...
    {
        $cekSession = $this->db->get_where('pelanggan', ['email' => $this->session->userdata('email')])->row_array();
        if ($cekSession != '') {
            if ($cekSession) {
                redirect('home');
            } else {
    ...
    private function _login()
    {
        ...
        $users = $this->db->get_where('pelanggan', ['email' => $email])->row_array();
    ...
    public function register()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $getLasCode = $this->db->query('SELECT MAX(kode) AS KODE FROM pelanggan')->row();
